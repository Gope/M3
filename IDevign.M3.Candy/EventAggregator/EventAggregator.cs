...
namespace IDevign.M3.Candy.EventAggregator
{
    ...
    public class EventAggregator : IEventAggregator
    {
        ...
        {
            // Design Time Support (Expression Blend)
        }
        ...
        public void Subscribe(object subscriber)
        {
            this.RunLocked(() =>
            {
                if (this.IsNotNull(subscriber) &&
                    this.IsNotYetInCollection(subscriber)&&
                    this.IsHandlerInterfaceImplemented(subscriber))
                {
            ...
            this.RunLocked(() =>
            {
                foreach (var subscriber in this.GetAllSubscribersFor<TMessage>())
                {
                    if (this._Dispatcher != null)
                        this._Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() => subscriber.Handle(message)));
                    else
        ...
        {
            var element = this._Subscribers.Select(o => o).Where(o => o.Target == subscriber).FirstOrDefault();
